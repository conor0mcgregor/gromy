...
class FirestoreFavoriteService implements FavoriteRepository {
  ...
  Stream<bool> isFavorite(String uid, String tournamentId) {
    return _userFavorites(
      uid,
    ).doc(tournamentId).snapshots().map((snapshot) => snapshot.exists);
  }
  ...
  Stream<List<AppTournament>> watchFavoriteTournaments(String uid) {
    return _userFavorites(
      uid,
    ).orderBy('createdAt', descending: true).snapshots().switchMap((snapshot) {
      if (snapshot.docs.isEmpty) {
        return Stream.value(<AppTournament>[]);
      }

      final tournamentIds = snapshot.docs.map((doc) => doc.id).toList();
      ...
      for (var i = 0; i < tournamentIds.length; i += 30) {
        chunks.add(
          tournamentIds.sublist(
            i,
            i + 30 > tournamentIds.length ? tournamentIds.length : i + 30,
          ),
        );
      }
        ...
        return _tournaments
            ...
            .map(
              (snap) => snap.docs
                  ...
                  .where((tournament) => tournament.isPubliclyVisible)
                  .toList(),
            );
      });
